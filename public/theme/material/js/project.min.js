...
$(function(){
	...
	$('.backtohome>a').hover(function(){
        $(this).find('i').addClass('auth-i-rotate');
	},function(){
		var irorate = $(this);
		setTimeout(function(){
           irorate.find('i').removeClass('auth-i-rotate');
		},200); 
	})
...
// 弹出层插件
	function custModel(el1,el2) {
		let button = el1;
		let model = el2;
		let mask = document.createElement('div');
		custModelOpen(button,model,mask);
		custModelClose(model,mask);
	      }
	
	
	      function custModelOpen(button,model,mask) {
		button.addEventListener('click',()=>{
		  insertCustMask(mask);
		  setTimeout(()=>{
		    insertCustModel(model);
		  },200);
		 });
	      }
	
	      function custModelClose(model,mask) {
		mask.addEventListener('click',()=>{
		    removeCustModel(model);
		    setTimeout(()=>{
		      removeCustMask(mask);
		    },300);
		}); 
	      ...
	      //插入遮罩
	      function insertCustMask(mask) {
		mask.classList.add('cust-mask');
		...
		setTimeout(()=>{
	      ...
	      //显示隐藏的model
	      function insertCustModel(model) {
		model.style.display = 'unset'
		setTimeout(()=>{
		  model.classList.add('cust-modelin');
		},100);
	      }
	      //隐藏model
	      function removeCustModel(model) {
		model.classList.remove('cust-modelin');
		setTimeout(()=>{
		  model.style.display = 'none';
		},300);
	      }
	      //隐藏并删除遮罩
	      function removeCustMask(mask) {

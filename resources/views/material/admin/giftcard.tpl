...
<div class="page-wrapper">
                            ...
                            <table id="data_table" class="table card-table table-vcenter text-nowrap datatable">
                                ...
                                <tbody id="table_content">
                                    {foreach $logs as $log}
                                        <tr>
                                            <td>
                                                <a class="text-red" href="#" onclick="deleteGiftCard('{$log->id}')">删除</a>
                                            </td>
                                            {foreach $details['field'] as $key => $value}
                                                <td>{$log->$key}</td>
                                            {/foreach}
    ...
    <div class="modal modal-blur fade" id="create-dialog" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn me-auto" data-bs-dismiss="modal">取消</button>
                    <button id="create-button" onclick="createOrUpdate('/admin/{$details['route']}', 'POST')"
    ...
    <script>
        function adjustStyle() {
            $("td:contains('已用')").css("color", "green");
            $("td:contains('未使用')").css("color", "orange");
                ...
                success: function(data) {
                    if (data.ret == 1) {
                        ...
                        for (var i = 0; i < data.result.length; i++) {
                            str += "<tr><td>" +
                                '<a class=\"text-red\" href="#" onclick="deleteGiftCard(' + data
                                .result[i].id + ')">删除</a>' +

              ...
              <!--首页开始-->
					<header id="header">
                       ...
                       {if $user->isLogin}
						<div class="content">
							<div class="inner">
                                  <p>Name: <code>{$user->user_name}</code>
                                    Grade: {if $user->class!=0}
											<code>VIP{$user->class}</code>
                                          {else}
                                             <code>Free</code>
                                              {/if}
                                    expire date: {if $user->class_expire!="1989-06-04 00:05:00"}
											    <code>{$user->class_expire}</code>
                                          {else}
                                              <code>Unlimited</code>
                                              {/if}</p>
                                  <p>Total traffic: <code>{$user->enableTraffic()}</code>
                                  Used: <code>{$user->usedTraffic()}</code>
                                  Left: <code>{$user->unusedTraffic()}</code></p>
                          </div>
                      </div>	
					  	<nav>
							<ul>
					...
					<div id="main">
                     ...
                     <!--标签4开始-->
                      <article id="4">
								...
								<ul class="icons">
                                   <p>Fill contact information here</p>
                                    <li>
                      	...
                      	<article id="login">  
								...
								<form method="post" action="javascript:void(0);">
									...
									<ul class="actions">
										<li><input id="login" type="submit" value="Login" class="special" /></li>

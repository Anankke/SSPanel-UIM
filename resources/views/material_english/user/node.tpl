...
{function displayV2rayNode node=null}
	...
	<p>Address: <span class="card-tag tag-blue">{$server_explode[0]}</span></p>

	<p>Port: <span class="card-tag tag-volcano">{$server_explode[1]}</span></p>

	<p>Protocol Parameter: <span class="card-tag tag-green">{$server_explode[0]}</span></p>

	<p>User UUID: <span class="card-tag tag-geekblue">{$user->getUuid()}</span></p>

	<p>Traffic Ratio: <span class="card-tag tag-red">{$node['traffic_rate']}</span></p>

	<p>AlterId: <span class="card-tag tag-purple">{$server_explode[2]}</span></p>

	<p>VMess Link: 
		<a class="copy-text" data-clipboard-text="{URL::getV2Url($user, $node['raw_node'])}">Copy</a>
	</p>
{/function}
						...
						{if $node['class']!=$class}
							...
							<div class="nodetitle">
								<a class="waves-effect waves-button" data-toggle="collapse" href="#cardgroup{$class}" aria-expanded="true" aria-controls="cardgroup{$class}">
								    <span>{if $class == 0}Ordinary{else}VIP {$node['class']} {/if}User node</span><i class="material-icons">expand_more</i>
								</a>
							</div>
						...
						<div class="node-tip cust-model" tipindex="{$node@index}">
								{if $node['class'] > $user->class}
									<p class="card-heading" align="center"><b> <i class="icon icon-lg">visibility_off</i>
										Your current level is not enough to use this node. If you need to upgrade, please <a href="/user/shop">click here</a>.</b></p>
								{else}
									...
									{if $node['mu_only'] != 1 && $node['sort'] != 11}
									    <div class="tiptitle">
											<a href="javascript:void(0);" onClick="urlChange('{$node['id']}',0,{if $relay_rule != null}{$relay_rule->id}{else}0{/if})">
												...
												<div class="nodeload">
													<div class="label label-brand-accent"> ↑Click node to view the details.</div>
												<div>
													<span class="node-icon"><i class="icon icon-lg">cloud</i></span>
													<span class="node-load">load:<code>{if $node['latest_load'] == -1}N/A{else}{$node['latest_load']}%{/if}</code></span>
												</div>
											</div>
										</div>
									{elseif $node['sort'] == 11}
										...
										{/if}
											<div class="tiptitle">
												<a href="javascript:void(0);" onClick="urlChange('{$node['id']}',{$single_muport['server']->server},{if $relay_rule != null}{$relay_rule->id}{else}0{/if})">{$node['name']}
													{if $relay_rule != null} - {$relay_rule->dist_node()->name}{/if} - Single Port Shadowsocks - {$single_muport['server']->server} Port
												</a>
											</div>
										{/foreach}
									{/if}
								...
								<div class="card-inner margin-bottom-no">
									<div class="tile-wrap">        
										...
										{if $node['class']!=$class}
											...
											<p class="card-heading">{if $class == 0}Ordinary{else}VIP {$node['class']} {/if}User node</p>	
										{/if}
											...
											<div data-toggle="tile" data-target="#heading{$node['id']}">
												...
												<div class="tile-inner">
													<div class="text-overflow node-textcolor">
											            ...
											            | <span class="node-icon"><i class="icon icon-lg">cloud</i></span>
														<span class="node-load">Load:{if $node['latest_load'] == -1}N/A{else}{$node['latest_load']}%{/if}</span> 
														...
														<span class="node-tr">{$node['traffic_rate']} Ratio</span> 
                                                 ...
                                                 <div class="card nodetip-table">
													...
													{if $node['mu_only'] != 1 && $node['sort'] != 11}
																...
																<p class="card-heading">
																	...
																	<span class="label label-brand-accent">←Click the node to view the details.</span>
																</p>
													...
													{foreach $nodes_muport as $single_muport}
														...
														{/if}
															...
															<p class="card-heading">
																<a href="javascript:void(0);" onClick="urlChange('{$node['id']}',{$single_muport['server']->server},{if $relay_rule != null}{$relay_rule->id}{else}0{/if})">{$node['name']}
																	{if $relay_rule != null} - {$relay_rule->dist_node()->name}{/if} - Single Port Shadowsocks -
																	{$single_muport['server']->server} Port</a><span class="label label-brand-accent">←Click the node to view the details.</span>
															</p>
...
<script>
	...
	$(".copy-text").click(function () {
		$("#result").modal();
		$("#msg").html("Copied, please open the software to add.");
	});
